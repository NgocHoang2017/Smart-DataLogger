...
//--------------------------------------------------------------------------------//
  ...
  if(next_1==1)
  {
    ...
    else
    {
      ...
      strcpy(data, "");
      strcpy(data, "C=loi&H=loi&S=loi&R=loi&M=loi3");//cap nhat he thong loi
...
void Slave1a()
{
  ...
  if(radio.write(&payload, sizeof(payload)))//neu truyen da thanh cong
  {
    //delay(2000);
  ...
  else
  {
    ...
    strcpy(data, "C=loi&H=loi&S=loi&M=loi&R=loi");//cap nhat he thong loi
    truyen_len_serve_slave3();
    //strcpy(data, "");//xoa cac ki tu trong data
  }
}
...
void rx_1b()
{
  if(radio.available())
  {
    ...
    while(b==0)
    {
      ...
      if(strcmp(data_b,"ON_1b")==0)//neu da dung trang thai yeu cau thi khong gui gui
      {
        if(radio.write(&ok, sizeof(ok)))
        {
          thoi_gian_cho=10000;//thoat slave
          ...
          //Serial.println(kiem_data);
          strcpy(data, "");//xoa cac ki tu trong kiem_data
          strcpy(data, kiem_data); // Gán chuỗi data có giá trị là chuỗi kiem_data
      ...
      else if(strcmp(data_b,"OFF_1b")==0)//neu da dung trang thai yeu cau thi khong gui gui
      {
        if(radio.write(&ok, sizeof(ok)))
        {
          thoi_gian_cho=10000;//thoat slave
      ...
      else if(strcmp(data_b,"Khong bat duoc bom_1b")==0 || strcmp(data_b,"Khong tat duoc bom_1b")==0)
      {
        thoi_gian_cho=10000;//thoat slave
        b=1;//thoat che do gui
        a=1;//thoat slave
...
void Slave2a()
{
  ...
  //doc max-min tu app//   
  //char check_2a[32] = "yeu cau slave 2a ";
  //strcat(check_2a, payload2); 
  //Serial.println(check_2a);
  a=0;
  b=0;
  c=0;
  next_1=0;
  ...
  //delay(1000);
  //-------------------------vao che do doc du lieu------------------------//
  if(radio.write(&payload2, sizeof(payload2)))//neu truyen da thanh cong
  {
    //delay(2000);
...
void rx_2b()
{
  if(radio.available())
  {
    ...
    while(b==0)
    {
      ...
      if(strcmp(data_b,"ON_2b")==0)//neu da dung trang thai yeu cau thi khong gui gui
      {
        if(radio.write(&ok, sizeof(ok)))
        {
          thoi_gian_cho=10000;//thoat slave
      ...
      else if(strcmp(data_b,"OFF_2b")==0)//neu da dung trang thai yeu cau thi khong gui gui
      {
        if(radio.write(&ok, sizeof(ok)))
        {
          thoi_gian_cho=10000;//thoat slave
      ...
      else if(strcmp(data_b,"Khong bat duoc bom_2b")==0 || strcmp(data_b,"Khong tat duoc bom_2b")==0)
      {
        thoi_gian_cho=10000;//thoat slave
        b=1;//thoat che do gui
        a=1;//thoat slave
...
void load(){
  ...
  //http.begin("http://192.168.1.8:80/doan/max_min.php");              //duong dan file php... vi dang ket noi wifi nen phai lay ip may thay vi localhost
  ...
  for (byte len = 2;len<=payload0.length()+1; len++)
  {
...
void load2(){
  ...
  //http.begin("http://192.168.1.8:80/doan/max_min2.php");              //duong dan file php... vi dang ket noi wifi nen phai lay ip may thay vi localhost
