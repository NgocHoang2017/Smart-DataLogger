...
//---------dia chi slave 1------------//
//unsigned char  CTR_TX_ADDRESS[CTR_TX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0x11};   //5 bytes
//unsigned char  CTR_RX_ADDRESS[CTR_RX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0x11};  //5 bytes
//unsigned char  CTR_TX_ADDRESSB[CTR_TX_ADR_WIDTH]= {0x33,0x33,0x33,0x33,0x33};   //5 bytes
//unsigned char  CTR_RX_ADDRESSB[CTR_RX_ADR_WIDTH]= {0x33,0x33,0x33,0x33,0x33};  //5 bytes
//---------dia chi slave 1------------//

//---------dia chi slave 2------------//
unsigned char  CTR_TX_ADDRESS[CTR_TX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0x22};   //5 bytes
unsigned char  CTR_RX_ADDRESS[CTR_RX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0x22};  //5 bytes
unsigned char  CTR_TX_ADDRESSB[CTR_TX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0xFF};   //5 bytes
unsigned char  CTR_RX_ADDRESSB[CTR_RX_ADR_WIDTH]= {0x11,0x11,0x11,0x11,0xFF};  //5 bytes
//---------dia chi slave 2------------//
...
void CTR_nrfInitP0(void)//reset
{
  ...
  CTR_spiRWreg(CTR_WRITE_RE + CTR_EN_AA, 0x01);      // EN P0, 2-->P1(chon luong p0) //----chi xai duoc luong 1,3,5 khi nhan
...
void CTR_nrfInit(void)
{
	...
	HAL_GPIO_WritePin(NRF_SCK_GPIO_Port,NRF_SCK_Pin,GPIO_PIN_RESET);//Spi clock line init high SCK=0
	CTR_spiWriteBuff(CTR_WRITE_RE + CTR_TX_ADDR, CTR_TX_ADDRESS, CTR_TX_ADR_WIDTH);    // 0x20 + 0x10, cau hinh dia chi truyen 3-5 bytes
  CTR_spiWriteBuff(CTR_WRITE_RE + CTR_RX_ADDR_P3, CTR_RX_ADDRESS, CTR_RX_ADR_WIDTH); // 0x20 + 0x0A,	cau hinh dia chi nhan
  CTR_spiRWreg(CTR_WRITE_RE + CTR_EN_AA, 0x03);      // EN P0, 2-->P1(chon luong p0) //----chi xai duoc luong 1,3,5 khi nhan
  CTR_spiRWreg(CTR_WRITE_RE + CTR_EN_RXADDR, 0x03);  //Enable data P0 
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RF_CH, 4);        // Chanel 1 RF = 2400 + RF_CH* (1 or 2 M) chon kenh 1
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RX_PW_P3, CTR_RX_PLOAD_WIDTH);  // Do rong data truyen 32 byte	 
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RF_SETUP, 0x07);        // 1M, 0dbm
  CTR_spiRWreg(CTR_WRITE_RE + CTR_CONFIG, 0x0e);         // Enable CRC, 2 byte CRC,power up, Send (cau hinh truyen)
}
void CTR_nrfInitB(void)//
{
	HAL_Delay(100);
	HAL_GPIO_WritePin(NRF_CE_GPIO_Port,NRF_CE_Pin,GPIO_PIN_RESET);//chip enable CE=0
	HAL_GPIO_WritePin(NRF_CSN_GPIO_Port,NRF_CSN_Pin,GPIO_PIN_SET);//Spi disable CSN=1
	HAL_GPIO_WritePin(NRF_SCK_GPIO_Port,NRF_SCK_Pin,GPIO_PIN_RESET);//Spi clock line init high SCK=0
	CTR_spiWriteBuff(CTR_WRITE_RE + CTR_TX_ADDR, CTR_TX_ADDRESSB, CTR_TX_ADR_WIDTH);    // 0x20 + 0x10, cau hinh dia chi truyen 3-5 bytes
  //CTR_spiWriteBuff(CTR_WRITE_RE + CTR_RX_ADDR_P2, CTR_RX_ADDRESSB, CTR_RX_ADR_WIDTH); // 0x20 + 0x0A,	cau hinh dia chi nhan
  CTR_spiRWreg(CTR_WRITE_RE + CTR_EN_AA, 0x02);      // EN P0, 2-->P1(chon luong p0)
  CTR_spiRWreg(CTR_WRITE_RE + CTR_EN_RXADDR, 0x02);  //Enable data P0 
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RF_CH, 18);        // Chanel 1 RF = 2400 + RF_CH* (1 or 2 M) chon kenh 1
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RX_PW_P2, CTR_RX_PLOAD_WIDTH);  // Do rong data truyen 32 byte	 
  CTR_spiRWreg(CTR_WRITE_RE + CTR_RF_SETUP, 0x07);        // 1M, 0dbm
  CTR_spiRWreg(CTR_WRITE_RE + CTR_CONFIG, 0x0e);         // Enable CRC, 2 byte CRC,power up, Send (cau hinh truyen)
}
...
void CTR_nrfSendPacketB(unsigned char*tx_buf)
{
	HAL_GPIO_WritePin(NRF_CE_GPIO_Port,NRF_CE_Pin,GPIO_PIN_RESET);//enable chip
	CTR_spiWriteBuff(CTR_WRITE_RE + CTR_RX_ADDR_P2, CTR_TX_ADDRESSB, CTR_TX_ADR_WIDTH);// Send Address
	CTR_spiWriteBuff(CTR_WR_TX_PLOAD, tx_buf, CTR_TX_PLOAD_WIDTH);//send data
	CTR_spiRWreg(CTR_WRITE_RE + CTR_CONFIG, 0x0e);// Send Out //Enable CRC, 2 byte CRC,power up, Transmit (cau hinh truyen)
	HAL_GPIO_WritePin(NRF_CE_GPIO_Port,NRF_CE_Pin,GPIO_PIN_SET);//disable chip
	CTR_spiRWreg(CTR_WRITE_RE + CTR_STATUS, 0xFF);
